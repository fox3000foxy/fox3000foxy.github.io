...
import { ALL_LANGS, LANG_LABELS, SITE_URL } from "../../lib/i18n";
...
${tagXml}    </item>`;
	...
	const altLinks = ALL_LANGS.filter((l) => l !== lang)
		.map(
			(l) =>
				`    <atom:link href="${siteUrl}/feed.${l}.xml" rel="alternate" type="application/rss+xml" hreflang="${l}" />`
		)
		.join("\n");
