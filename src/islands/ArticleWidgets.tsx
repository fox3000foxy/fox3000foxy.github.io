    ...
    <div className="article-widgets-bottom">
      ...
      {meta?.tags && meta.tags.length > 0 && (
        <LazyVisible rootMargin="400px" height="200px">
          <SuggestedArticles currentSlug={slug} currentTags={meta.tags} allArticles={allArticles} lang={lang} />
        </LazyVisible>
      )}
      {(prevArticle || nextArticle) && (
        <nav className="article-nav">
          {prevArticle && (
            <Link to={`/blog/${prevArticle.slug}`} className="article-nav-link prev">
              <span className="article-nav-label">{t("article.prev")}</span>
              <span className="article-nav-title">{prevArticle.title}</span>
            </Link>
          )}
          {nextArticle && (
            <Link to={`/blog/${nextArticle.slug}`} className="article-nav-link next">
