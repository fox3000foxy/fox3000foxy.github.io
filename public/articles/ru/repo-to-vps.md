...
Но ничто не мешает тебе... заняться чем-то другим на этой VM. Например, открыть SSH-шелл и использовать её как сервер.
...
Вот тут начинается гениальный хак.
...
# забираем ветку filesystem из remote
git fetch origin "$filesystem_branch":refs/remotes/origin/$filesystem_branch

# восстанавливаем workspace из этой ветки
...
Это та деталь, которую не замечаешь с первого взгляда, но именно она отличает "работает" от "РАБОТАЕТ НА УРА".
...
autosave() {
  while inotifywait -qq -r -e modify,create,delete,move \
    ...
    commit_and_push
    sleep 1   # debounce на случай шквала изменений
  done
}
...
periodic_save() {
  while true; do
    sync_from_remote   # забираем возможные удалённые изменения
    sleep 5
    commit_and_push
  done
}
...
commit_and_push() {
  (
    flock -n 200 || return   # блокировка, чтобы два сохранения не запустились одновременно

    git add -A
    git reset -- .github/workflows/ .github/scripts/   # не трогаем скрипты

    if ! git diff --cached --quiet; then
      if git rev-parse --verify HEAD >/dev/null 2>&1; then
        git commit --amend --no-edit    # AMEND: перезаписываем предыдущий commit
      else
...
Всё взаимосвязано. Чувак продумал полный жизненный цикл.
...
Но скрипт всё ещё нужен во время сессии (чтобы обновлять README при каждом перезапуске tmate). Поэтому он прячет их в `/tmp`, вне досягаемости git, чтобы вызывать позже:

```bash
python3 "$RUNNER_SCRIPTS_DIR/scripts/update_readme.py" --ssh "$tmate_ssh" ...
```

Это тот тип бага, который выстрелит тебе в лицо, если не подумать: "почему мой скрипт исчез?". А он подумал.
...
sudo cp "$HOME/.bashrc" /root/.bashrc   # то же самое для root через sudo
```
...
# Ctrl+D делает то же, что и exit
bind -x '"\C-d": "exit"'
```
...
while true; do
  tmate -S /tmp/tmate.sock new-session -d "bash --rcfile $HOME/.bashrc -i"
  # ... генерирует ссылки, обновляет README ...

  # ждём, пока сессия tmate умрёт
...
Это та деталь, которая превращает хак-поделку в реально юзабельную штуку.
...
Но для временного Linux-окружения, которое можно восстанавливать по желанию? Это просто гениально.
...
И `git commit --amend` + force-push — это общая техника: **ты держишь один commit, представляющий текущее состояние, перезаписываемый при каждом обновлении.** Никакой раздутой истории, просто живой снапшот.

Изначально это не для этого создавалось. Но это работает. И это бесплатно. И в этом вся красота.
