---
title: "Luna Protocol: İnsanı taklit eden bağımsız bir Discord botu oluşturdum"
description: "Luna Protocol, yerel LLM ile tamamen bağımsız bir Discord botudur; uyku, yazım hataları, tereddütler, unutkanlıklar, tematik yorgunluk ve spontane mesajlarla doğal konuşma yapabilir."
date: 2026-07-11
tags:
  - discord-bot
  - llm
...
author_sig: "db8MsHJKYmJOObSI/Sb2A+YADtGZe8tcNdpFhfy+i3ee1lglUUXwLqz1IbrRTDMncZ+g3dretbfk89FRruOpAQ=="
---

# Luna Protocol: İnsanı taklit eden bağımsız bir Discord botu oluşturdum
Bir Discord botu **uyuyabilse**, **yazım hataları yapsa**, **tereddüt etse**, **yanıt vermeyi unutsa** ve bazen kendi inisiyatifiyle size mesaj gönderse? İşte **Luna Protocol**'ün yaptığı tam olarak bu: Yerel bir LLM (llama.cpp) çalıştıran ve kusurlu bir insan gibi sohbet eden tamamen bağımsız bir Discord botu.
Sert prompt'lar yok, robotik yanıtlar yok. Luna'nın **öncelikli tetikleme sistemi**, **değişken gecikmeler**, **uyku çizelgesi**, **spontane mesajlar** ve hatta sesli mesaj göndermek için bir **TTS pipeline**'ı var. Hepsi basit bir `config.yml` dosyası ile yeniden yüklenebilir şekilde yapılandırılabilir.
Bu makalede, tam mimariyi inceliyoruz: Generic event bus'tan TTS pipeline'a, tetikleme sisteminden, insan bileşenlere ve fine-tuning veri setine kadar.
![Mimari Genel Bakış -- küresel bileşenler ve veri akışı](/images/luna-protocol/01-architecture-overview.svg)

---

## Mimari: tipolmuş olay busu
...
## LLM boru hattı: iki mod

### 모드 `direct` (défaut)

Bot, istekleri doğrudan yerel `llama-server`'a HTTP üzerinden gönderir. Model paylaşılır, prompt cache ve 4 eşzamanlı slot ile. İki PM2 süreci: LLM sunucusu ve bot istemcisi.
...
`state-machines/` klasörü **24 Mermaid diyagramı** içerir ve tüm kaynak kodunu kapsar. Her diyagram, insan dilinde ayrıntılı bir açıklamaya sahiptir.
