...
const AUTHOR_PUBKEY =
	"MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE+nMe+wl5gkHk05/0EQ000PcDswTNZmZKqNtITRNVV+GGoarYNBDZxIYk2KbjUkdkmuqUhuAXmuXCG3GT7+1U6Q==";
...
async function signArticle(
	...
	const keyData = Uint8Array.from(atob(privateKeyBase64), (c) =>
		c.charCodeAt(0)
	);
...
async function ensureFork(
	...
	const res = await fetch(`https://api.github.com/repos/${userFork}`, {
		headers,
	});
	if (res.ok) {
		return userFork;
...
async function createBranch(
	...
	const {
		object: { sha: mainSha },
	} = (await mainRef.json()) as { object: { sha: string } };
...
async function submitArticle(request: Request, env: Env): Promise<Response> {
	...
	if (article.images?.length) {
		...
		if (failedImages.length) {
			return json(
				{ error: `Failed to upload images: ${failedImages.join(", ")}` },
				502
			);
		}
	}
