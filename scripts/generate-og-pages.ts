...
function main() {
	...
	for (const [slug, info] of bySlug) {
		...
		const hreflangTags = info.langs
			.map(
				(l) =>
					`<link rel="alternate" hreflang="${l}" href="${url}?lang=${l}" />`
			)
			.join("\n");
		...
		const jsonLd = JSON.stringify({
			...
			publisher: {
				...
				logo: {
					"@type": "ImageObject",
					url: "https://fox3000foxy.com/icons/apple-icon-180x180.png",
				},
			},
		...
		const html = baseHtml
			...
			.replace(
				/<title>[^<]*<\/title>/,
				`<title>${escapeXml(info.title)} | Fox's Blog</title>`
			)
			.replace(
				"</head>",
	...
	console.log(
		`Generated ${count} pages (${rendered} PNGs rendered, ${skipped} cached)`
	);
}
