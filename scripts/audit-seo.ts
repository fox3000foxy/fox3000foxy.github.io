...
function checkRss() {
	...
	if (!xml.includes('<?xml version="1.0"')) {
		fail("RSS", "Missing XML declaration");
	}
	if (!xml.includes('<rss version="2.0"')) {
		fail("RSS", "Not RSS 2.0");
	}
	...
	if (!items || items.length === 0) {
		...
		ok("RSS", `${items.length} items`);
	}

	// Channel-level elements
	if (!xml.includes("<title>")) {
		fail("RSS", "Missing channel <title>");
	}
	if (!xml.includes("<link>")) {
		fail("RSS", "Missing channel <link>");
	}
	if (!xml.includes("<description>")) {
		fail("RSS", "Missing channel <description>");
	}
	if (!xml.includes("<language>")) {
		warn("RSS", "Missing channel <language>");
	}
	if (!xml.includes("<lastBuildDate>")) {
		warn("RSS", "Missing <lastBuildDate>");
	}
	if (!xml.includes("<atom:link")) {
		warn("RSS", "Missing <atom:link self>");
	}
	if (!xml.includes("<image>")) {
		warn("RSS", "Missing <image> (recommended for readers)");
	}
	if (!xml.includes("<copyright>")) {
		warn("RSS", "Missing <copyright>");
	}
	if (!xml.includes("<managingEditor>")) {
		warn("RSS", "Missing <managingEditor>");
	}
	...
	for (const block of itemBlocks) {
		...
		if (!block.includes("<title>")) {
			fail("RSS", "Item missing <title>");
		}
		if (!block.includes("<link>")) {
			fail("RSS", "Item missing <link>");
		}
		if (!block.includes("<guid")) {
			fail("RSS", "Item missing <guid>");
		}
		if (!block.includes("<pubDate>")) {
			fail("RSS", "Item missing <pubDate>");
		}
		if (!block.includes("<description>")) {
			warn("RSS", "Item missing <description>");
		}
		...
		if (matchDate) {
			...
			if (Number.isNaN(d.getTime())) {
				fail("RSS", `Invalid pubDate: ${matchDate[1]}`);
			}
		}
	}
	...
	const langDirs = fs
		.readdirSync("public/articles")
		.filter(
			(d) => fs.statSync(`public/articles/${d}`).isDirectory() && d !== "assets"
		);
	...
	for (const [slug, count] of seenSlugs) {
		if (count > 1 && count < expectedCount * 0.5) {
			warn(
				"RSS",
				`Article "${slug}" appears ${count}x (not all ${expectedCount} langs)`
			);
		}
	}
	...
	if (multiLangCount > 0) {
		warn(
			"RSS",
			`${multiLangCount} articles appear in multiple languages in a SINGLE feed. RSS readers expect one language per feed. Google News requires separate feeds per language.`
		);
	}
	...
	if (lang && lang[1] === "en" && multiLangCount > 0) {
		warn(
			"RSS",
			"Feed says <language>en</language> but contains articles in multiple languages"
		);
	}
}
...
function checkNewsSitemap() {
	...
	if (!fs.existsSync(file)) {
		fail("NewsSitemap", "news-sitemap.xml not found");
		return;
	}
	...
	if (
		!xml.includes('xmlns:news="http://www.google.com/schemas/sitemap-news/0.9"')
	) {
	...
	if (!urls) {
		ok("NewsSitemap", "No articles < 48h old");
		return;
	}
	...
	for (const urlBlock of urls) {
		const dateMatch = urlBlock.match(
		...
		if (!dateMatch) {
			fail("NewsSitemap", "URL missing <news:publication_date>");
			continue;
		}
		if (!titleMatch) {
			fail("NewsSitemap", "URL missing <news:title>");
			continue;
		}
		...
		if (pubDate < twoDaysAgo) {
			warn(
				"NewsSitemap",
				`"${titleMatch[1].slice(0, 40)}..." published ${dateMatch[1]}. Google News only accepts articles < 48h old.`
			);
		}
		...
		if (!langMatch) {
			warn(
				"NewsSitemap",
				`"${titleMatch[1].slice(0, 40)}..." missing <news:language>.`
			);
		}
...
function checkSitemap() {
	...
	for (const lang of langDirs) {
		if (!xml.includes(`?lang=${lang}`) && lang !== "en") {
			warn(
				"Sitemap",
				`No URL with ?lang=${lang} found. Language-specific pages should be in sitemap.`
			);
		}
	}
...
function checkSpaSeo() {
	...
	if (html.match(/<html[^>]+lang=/)) {
		ok("SPA", "html lang attribute found");
	} else {
		warn("SPA", "Missing <html lang> attribute");
	}

	if (!html.includes('rel="canonical"')) {
		warn("SPA", "Missing canonical link in <head>");
	}
	if (!html.includes('rel="alternate"')) {
		warn("SPA", "Missing hreflang alternate links in <head>");
	}
	...
	if (
		!(
			html.includes('"@type":"Blog"') ||
			html.includes('"@type":["Blog"') ||
			html.includes('"@type": "Blog"')
		)
	) {
		warn("SPA", "No Blog JSON-LD in index.html");
	}
	if (!html.includes('"@context":"https://schema.org"')) {
		warn("SPA", "No schema.org context in JSON-LD");
	}
...
function checkOgJsonLd() {
	...
	const articleDirs = fs
		.readdirSync(blogDist)
		.filter((d) => fs.statSync(path.join(blogDist, d)).isDirectory());
	for (const slug of articleDirs) {
		const html = fs.readFileSync(
			path.join(blogDist, slug, "index.html"),
			"utf8"
		);
		...
		if (!jsonLdMatch) {
			fail("OG-JSONLD", `${slug}: missing JSON-LD`);
			continue;
		}
		try {
			const parsed = JSON.parse(jsonLdMatch[1]);
			if (!parsed.headline) {
				fail("OG-JSONLD", `${slug}: missing headline`);
			}
			if (!parsed.datePublished) {
				warn("OG-JSONLD", `${slug}: missing datePublished`);
			}
			if (parsed.inLanguage === "multiple") {
				warn(
					"OG-JSONLD",
					`${slug}: inLanguage is "multiple", should be specific language`
				);
			}
			if (!parsed.author) {
				fail("OG-JSONLD", `${slug}: missing author`);
			}
			checked++;
		} catch {
